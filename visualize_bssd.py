# Visulisierung aller ways der XML-Datei über die GUI
...
def visualize(bssd):
    ...
    # Visualisiert alle ways nach der Reihenfolge der ID-Nummern
    if bssd.find('way'):
        list_ways = bssd.findall('way')
        list_ids = []
        ...
        list_ways_sorted = list_sort(list_ways, list_ids)

        for way in list_ways_sorted:
            div_viz.append(visualize_way_info(way))
            div_viz.append(visualize_way(way))

    # graut das HTML-Element für die Schaltflächen für Lichtzeichenanlagen aus
...
def open_html(bssd):
    ...
    template.stream(viz=viz_txt).dump('output/output.html')

    # Open HTML file
    filename = 'file:///' + os.getcwd() + '/output/' + 'output.html'
    webbrowser.open_new_tab(filename)


#tree = ET.parse("demo_map.xml")
#bssd = tree.getroot()
#open_html(bssd)
